...
export default function Calendar({ events, onEventClick, toggleModal }: Props) {

    console.log(events)
    if (events?.length) { console.log(events[0]?.start.toISOString()) }
                ...
                <ol className="mt-4 space-y-1 text-sm leading-6 text-gray-500">
                    {events?.map((event) => (
                        <li
                            ...
                            className="group flex items-center space-x-4 rounded-xl py-2 px-4 focus-within:bg-gray-100 hover:bg-gray-100"
                        >
                            {/* <img src={.imageUrl} alt="" className="h-10 w-10 flex-none rounded-full" /> */}
                            <div className="flex-auto">
                                <p className="text-gray-900">{event.title}</p>
                                <p className="mt-0.5">
                                    <time dateTime={event.start.toISOString()}>{event.start.getHours()}</time> -{' '}
                                    <time dateTime={event.end.toISOString()}>{event.end.getHours()}</time>
                                </p>
                            </div>
                            <Menu as="div" className="relative opacity-0 focus-within:opacity-100 group-hover:opacity-100">
                                <div>
